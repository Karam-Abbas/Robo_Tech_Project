...
class MapHandler:
    ...
    def generate_map(self, num_clusters=20, min_cluster_size=10, max_cluster_size=45):
    ...
    def add_noise_to_distances(self, noise_range=(-3, 5), valid_range=None):
        ...
        The noise is constrained to integer values between -1 and 1.
        The valid_range dynamically depends on self.row and self.cols if not provided.
        """
        # Set valid range based on map dimensions if not explicitly provided
        ...
        for row in data:
            ...
            # Generate integer noise for each distance measurement
            noise = np.random.randint(noise_range[0], noise_range[1] + 1, size=4)  # Include noise_range[1]
